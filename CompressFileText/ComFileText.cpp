...
void HuffmanCompress(FILE *fileInput , FILE *fileOut , HuffMap &map , HuffData  data, int size)
{
	...
	while (!feof(fileInput))
	{
		...
		fread(&ch[0], sizeof(char), 1, fileInput);
	}
	...
	for (int z = 0; z < countBit; z++)
	{
		...
		op[1] = '\0';
		op[0] = bit8[z];
		fwrite(op, sizeof(char), 1, fileInput);


		soBitLe++;
	}

	fwrite(&(soBitLe), sizeof(int), 1, fileInput);
...
void ReadHeaderFile(FILE* fileIN, HuffData& data)
{
	...
	while (ch != '~')
	{
		...
		//cout << data.s[i] << " Va " << data.wei[i] << endl;
...
void ConvertToBinArray(FILE* in, int indexStart, int* a , long long &k , long long viTriLe)
{
	...
	cout << "INDEX STARTzz " << indexStart << endl;
	int count = ftell(in);
	/////////////DANH DAU
	char ch[2];
	ch[1] = '\0';
...
void Decode(FILE* in, FILE* out)
{
	...
	for(int z = 0; z < SoLuongBitLe; z++)
		vtLe -= 1;
	cout << "VT LE : " << vtLe << endl;

	rewind(in);

	cout << "POS REWIND " << ftell(in) << endl;
